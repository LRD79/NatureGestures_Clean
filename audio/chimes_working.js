...
function tryPlayBurst(current) {
  ...
  // 🌟 Get current weather-mapped key
  ...
  const rate = keyToRate[currentKey] || 1.0;
  ...
  for (let i = 0; i < burst; i++) {
    ...
    const player = new Tone.Player({
      url: clip.url,
      playbackRate: rate
    }).connect(preGain).connect(pan).connect(chimeGain);
    player.start(Tone.now() + delay);
    setTimeout(() => player.dispose(), 10000);
  }

  console.log(`🔔 Burst of ${burst} chimes (key: ${currentKey}, rate: ${rate})`);
}
